    ...
    public String apply(String[] args) {
        if (args.length != 3) {
            throw new WrongCommandException("Wrong command syntax. "
                    + "use \"sell (user id) (stock symbol) (quantity)\""
            );
        }
        stockService.sellStock(userService.getUser(Integer.parseInt(args[0])), new StockSymbol(Integer.parseInt(args[1])), Long.parseLong(args[2]));
        return "sell " + Arrays.toString(args);
    }
